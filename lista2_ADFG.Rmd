...
## (d)
```{r PCA}
# Selecionando os retornos isoladamente no dataframe
portfolio_names <- c("SMALL LoBM_excess", "SMALL HiBM_excess", "ME1 BM2_excess", "ME1 BM3_excess",    "ME1 BM4_excess", "ME2 BM1_excess", "ME2 BM2_excess", "ME2 BM3_excess", "ME2 BM4_excess",
   "ME2 BM5_excess", "ME3 BM1_excess", "ME3 BM2_excess", "ME3 BM3_excess", "ME3 BM4_excess",
   "ME3 BM5_excess",  "ME4 BM1_excess", "ME4 BM2_excess", "ME4 BM3_excess", "ME4 BM4_excess", 
   "ME4 BM5_excess",  "ME5 BM2_excess", "ME5 BM3_excess",    "ME5 BM4_excess",  "BIG LoBM_excess",
   "BIG HiBM_excess")
RR_pca <- analysis_df %>%
  select(portfolio_names)

# Realizando a análise em componentes principais (normalizada)
pca_result <- prcomp(RR_pca, center = TRUE, scale. = TRUE)

# Extraindo o sumário dos resultados do PCA
pca_summary <- summary(pca_result)

# Determinando o número de fatores que explicam 60% ou mais da variância
cumulative_variance <- pca_summary$importance["Cumulative Proportion", ]
qnts_fatores <- which(cumulative_variance >= 0.60)[1]

print(pca_summary$importance[,1:10])

cat(sprintf(
  "Fatores necessários para explicar 60%% da variação: %d",
  qnts_fatores
))

# Guardando os cinco primeiros fatores, explicam 95% da variação
pca_factors <- as.data.frame(pca_result$x[, 1:5])
```
O primeiro fator é suficiente para explicar mais de 70% da variância do modelo, mas usaremos os primeiros cinco para melhor efeito de comparação mais a frente.

## (e)
```{r prep comparação}

#--------------- Loop para calcular os prêmios estimados e o R2 do modelo PCA--------------------

# Tabela pros prêmios e R2
pca_model_results <- data.frame(
  Portfolio = character(),
  R_Squared = numeric(),
...
for (portfolio in portfolio_names) {
  ...
  # Combinar os dados para a regressão: 
  # A variável dependente (retorno do portfólio) e as independentes (fatores PCA)
  regression_data <- cbind(
    Portfolio_Return = analysis_df[[portfolio]], 
    pca_factors
  )
  
  # 3. Rodar a regressão linear do retorno do portfólio contra os 5 fatores PCA
  # A fórmula "Portfolio_Return ~ ." significa regredir a primeira coluna contra todas as outras.
  model <- lm(Portfolio_Return ~ ., data = regression_data)
  
  # Extrair o sumário do modelo
  model_summary <- summary(model)
  
  # 4. Extrair o R² ajustado
  r_squared <- model_summary$r.squared
  
  # 5. Calcular o Equity Premium
  # O prêmio de risco estimado é a média dos valores preditos (fitted values) pela regressão.
  equity_premium <- mean(fitted(model))
  
  # 6. Adicionar os resultados ao dataframe
  pca_model_results <- rbind(
    pca_model_results,
    data.frame(
      Portfolio = portfolio,
      R_Squared = r_squared,
...
print("Resultados do Modelo de 5 Fatores (PCA)")
print(pca_model_results)

# Para comparação, você pode calcular o R² médio
cat(sprintf(
  "\nR² Médio do modelo PCA: %.4f",
  mean(pca_model_results$R_Squared)
))

#---------------Loop para obter um R2 para o modelo de consumo-----------------------

# Dataframe para o R2
ccapm_results <- data.frame(
  Portfolio = character(),
  R_Squared = numeric(),
  stringsAsFactors = FALSE
)
...
# Loop da regressão
for (portfolio in portfolio_names) {
  formula_str <- paste0("`", portfolio, "` ~ consumption_growth_pc")
  formula_obj <- as.formula(formula_str)
  
  # Rodar a regressão linear usando a fórmula criada
  model <- lm(formula_obj, data = analysis_df)
  model_summary <- summary(model)
  # Guardar o R2 aqui
  r_squared <- model_summary$r.squared
  # Adicionar os resultados ao nosso dataframe de resultados
  ccapm_results <- rbind(
    ccapm_results,
    data.frame(
      Portfolio = portfolio,
...
print(ccapm_results)
cat(sprintf(
  "\nR2 Médio do modelo básico: %.4f",
  mean(ccapm_results$R_Squared)
))
...
```{r comparação}
# Recolhendo os prêmios 
obs_EP <- sapply(analysis_df[portfolio_names], mean)
portfolio <- colnames(ff_quarterly_real %>% select(-YearQuarter, -comp_inflation_quarterly))
basico <- model_premiums_long['Model_Estimated_Premium']
capm <- capm_risk_premiums['CAPM_Premium']
tres_fatores <- three_factor_premiums['Three_Factor_Premium']
pca <- pca_model_results['Equity_Premium']

# Comparação dos equity premiums 
comp_EP <- cbind(portfolio, basico, capm, tres_fatores, pca, obs_EP)
colnames(comp_EP) <- c('Portfólio', 'Est. Consumo', 'Est. CAPM', 'Est. 3f', 'Est. PCA', 'Observado')

rownames(comp_EP) <- NULL
  
print(comp_EP)

# ------------------------------ Fazendo mesmo para o R2 -----------------------------
# CAPM
capm_summary <- capm_data %>%
  pivot_longer(
    cols = ends_with("_excess"),
    names_to = "Portfolio",
    values_to = "Excess_Return"
  ...
  # Expanda a tabela de estatísticas
  unnest(glanced)

# 3f
three_factor_summary <- three_factor_data %>%
  pivot_longer(
    cols = ends_with("_excess"),
    names_to = "Portfolio",
    values_to = "Excess_Return"
...
comp_R2 <- data.frame(
  ...
  R2_Modelo_Basico = ccapm_results$R_Squared,
  R2_CAPM = capm_summary$r.squared,
  R2_3_Fatores = three_factor_summary$r.squared,
  R2_PCA = pca_model_results$R_Squared
)

print(comp_R2)

# Tabela de R2 médios
medios_R2 <- data.frame(
  Modelo = c(
    "Consumo", 
    "CAPM", 
    "3f", 
  ...
  R2_Medio = c(
    mean(comp_R2$R2_Modelo_Basico),
    mean(comp_R2$R2_CAPM),
    mean(comp_R2$R2_3_Fatores),
    mean(comp_R2$R2_PCA)
  )
)

medios_R2$R2_Medio <- format(round(medios_R2$R2_Medio, 4), nsmall = 4)
print(medios_R2)
```
Temos observações de grau de ajuste crescente do modelo de consumo para o PCA, sendo esse último muito bem ajustado, mas de difícil interpretação econômica (por se tratar de um método estatístico de obtenção de fatores de maior poder explicativo).

## (f)
```{r fama-macbeth}
# Limpando o nome da coluna do portfólio para facilitar a junção posterior
betas_3f <- betas_3f %>%
  mutate(Portfolio = str_remove(Portfolio, "_excess"))

## Regressão cross-section

# Organizando os dados
famac_data <- three_factor_data %>%
  pivot_longer(
    cols = ends_with("_excess"),
    names_to = "Portfolio",
    values_to = "Excess_Return"
  ...
  mutate(Portfolio = str_remove(Portfolio, "_excess")) %>%
  # Juntar os betas estimados 
  left_join(betas_3f, by = "Portfolio")

# Realizando a regressão
famac_results <- famac_data %>%
  group_by(YearQuarter) %>%
  nest() %>%
  mutate(
    # Para cada trimestre, rodamos a regressão dos 25 retornos contra os 25 betas
    cross_section_model = map(data, ~ lm(Excess_Return ~ beta_mkt + beta_smb + beta_hml, data = .x)),
    # Extraímos os coeficientes (os gammas) de cada modelo
    tidied = map(cross_section_model, tidy)
  ) %>%
  # Expandir a tabela de coeficientes
  unnest(tidied)

# Prêmio de risco por trimestre:
print(head(famac_results), digits = 3)

# Agrupamos por fator (term) e calculamos a média, o erro padrão e a estatística-t
final_premiums_summary <- famac_results %>%
  group_by(term) %>%
  summarise(
    # Média do gamma (prêmio de risco médio)
    Avg_Premium = mean(estimate),
    # Erro padrão da média (desvio padrão / raiz do número de observações)
    Std_Error = sd(estimate) / sqrt(n()),
    # Estatística t de Fama-Macbeth
    T_Statistic = Avg_Premium / Std_Error,
    # P-valor do teste t
    P_Value = 2 * pt(-abs(T_Statistic), df = n() - 1)
  ) %>%
  rename(Fator = term)

print(final_premiums_summary, digits = 4,
            caption = "Prêmios de Risco Médios e Testes de Significância")
```
Analisando os p-valores da regressão final, apenas o beta do mercado é significativo, ou seja, é a única fonte de risco sistemático em que o efeito é estatisticamente diferente de zero. Além disso, o intercepto não ser estatisticamente diferente de zero corrobora a hipótese de uma série estacionária. 
