...
pacman::p_load(tidyverse, knitr, zoo, lubridate, broom, ipeadatar, dplyr, ggplot2, forecast, tseries, urca, plotly, writexl)
```
...
ff_monthly <- ff_raw %>%
  set_names(replace(names(.), 1, "Date")) %>%
...
cpi_monthly <- cpi_raw %>%
  ...
  arrange(Date) %>%
  mutate(Inflation_pct = ((CPI / lag(CPI)) - 1)) %>%
...
ff_quarterly_real <- ff_monthly_real %>%
  ...
  summarise(
    comp_inflation_quarterly = safe_prod(Inflation_pct) - 1,
    across(
      .cols = -c(Date, Inflation_pct),
      .fns = ~ (safe_prod(.x) / safe_prod(Inflation_pct)) - 1
      )
  ) %>%
  ungroup()

cat("Dataframe 'ff_quarterly_real' salvo com sucesso como ff_quarterly_real.xlsx\n")

ff_quarterly_real %>%
  mutate(comp_inflation_quarterly = round(comp_inflation_quarterly, 2)) %>%
  tail()
...
rf_real_quarterly <- rf_data_with_inflation %>%
  mutate(
    RF_real_quarterly = (( (1 + RF_quarterly) / (1 + comp_inflation_quarterly) ) - 1)
  ) %>%
...
model_premiums <- analysis_df %>%
  summarise(across(
    ...
    .fns = ~ -cov(SDF, .x, use = "complete.obs") / mean_sdf * 100,
    .names = "{.col}"
  ))
